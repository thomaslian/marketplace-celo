...
const connectCeloWallet = async () => {
  if (window.celo) {
    // Check if Celo wallet is installed
...
const getProducts = async () => {
  ...
  for (let i = 0; i < productsLength; i++) {
    let product = new Promise(async (resolve, reject) => {
      let p = await contract.methods.products(i).call(); // Use the getProduct method in the contract
      resolve({
        // Resolve the promise to return product data
        index: i,
        owner: p[0],
        name: p[1],
        image: p[2],
  ...
  return `
        <div class="card border-secondary mb-4">
            ...
            <div class="position-absolute top-0 end-0 ${
              productSold ? `bg-danger text-white` : `bg-warning`
            } mt-4 px-2 py-1 rounded-start">
            ...
            <div class="card-body text-left p-4 position-relative">
                ...
                <a class="btn btn-lg buyBtn fs-6 p-3 ${
                  productSold ? "btn-secondary disabled" : "btn-outline-dark"
                }" id=${product.index}>
